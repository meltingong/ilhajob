...
public class UserController {
	...
	public String user_view(HttpServletRequest request) throws Exception {
		String forwardPath="";
		/************** login check **************/
		//인터셉터? 03-06 AuthLogininterceptor 클래스 확인
...
//		}
	...
	public String user_modify_action(User user) throws Exception {
		String forwardPath = "";
		/************** login check **************/
		//커스텀 필터로 등록후에 인자로 request 필요없어짐
		/*****************************************/
	...
	public String user_remove_action(HttpServletRequest request) throws Exception {
		...
		forwardPath="redirect:user_main";
		//case 2
//		forwardPath="forward:user_logout_action"; //forward 붙여야 컨트롤러매핑에서 url찾음
		return forwardPath;
	}
	@LoginCheck
	...
	public String user_logout_action(HttpServletRequest request) {
		...
		/************** login check **************/
		/*****************************************/
		request.getSession(false).invalidate();// getSession(false) 일때는 세션이 존재하지않으면 null반환받음
		forwardPath="redirect:user_main";
		return forwardPath;
	}
